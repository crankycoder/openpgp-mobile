...
openpgp_result_t openpgp_sign_file(const char *input_file,
    ...
    /* Input validation */
    if (!input_file) {
        return create_error_result(OPENPGP_ERROR_INVALID_INPUT, "Input file path cannot be null");
    }
    if (!private_key) {
        return create_error_result(OPENPGP_ERROR_INVALID_INPUT, "Private key cannot be null");
    }
    ...
    /* Add input field */
    flatbuffers_string_ref_t input_ref = flatbuffers_string_create_str(&builder, input_file);
    model_SignFileRequest_input_add(&builder, input_ref);
    
    /* Add private_key field */
    flatbuffers_string_ref_t private_key_ref = flatbuffers_string_create_str(&builder, private_key);
    model_SignFileRequest_private_key_add(&builder, private_key_ref);
    
    /* Add passphrase field if provided */
    if (passphrase) {
        flatbuffers_string_ref_t passphrase_ref = flatbuffers_string_create_str(&builder, passphrase);
        model_SignFileRequest_passphrase_add(&builder, passphrase_ref);
    }
    
    /* Add options field if provided */
    if (options) {
        model_KeyOptions_ref_t key_options_ref = build_key_options(&builder, options);
        model_SignFileRequest_options_add(&builder, key_options_ref);
    }
    
    model_SignFileRequest_end_as_root(&builder);
    ...
    if (!buffer) {
        flatcc_builder_clear(&builder);
        return create_error_result(OPENPGP_ERROR_SERIALIZATION,
                                 "Failed to serialize SignFileRequest");
    }
    
    /* Call bridge */
    BytesReturn* response = g_openpgp.bridge_call("signFile", buffer, size);
...
openpgp_result_t openpgp_sign_bytes(const uint8_t *data, size_t data_len,
    ...
    /* Call bridge */
    BytesReturn* response = g_openpgp.bridge_call("signBytesToString", buffer, size);
...
openpgp_result_t openpgp_sign_data_bytes(const uint8_t *data, size_t data_len,
    ...
    /* Add message field (as byte vector) */
    flatbuffers_uint8_vec_ref_t message_ref = flatbuffers_uint8_vec_create(&builder, data, data_len);
    model_SignDataBytesRequest_message_add(&builder, message_ref);
    
    /* Add private_key field */
    flatbuffers_string_ref_t private_key_ref = flatbuffers_string_create_str(&builder, private_key);
    model_SignDataBytesRequest_private_key_add(&builder, private_key_ref);
    
    /* Add passphrase field if provided */
    if (passphrase) {
        flatbuffers_string_ref_t passphrase_ref = flatbuffers_string_create_str(&builder, passphrase);
        model_SignDataBytesRequest_passphrase_add(&builder, passphrase_ref);
    }
    
    /* Add options field if provided */
    if (options) {
        model_KeyOptions_ref_t key_options_ref = build_key_options(&builder, options);
        model_SignDataBytesRequest_options_add(&builder, key_options_ref);
    }
    
    model_SignDataBytesRequest_end_as_root(&builder);
    ...
    if (!buffer) {
        flatcc_builder_clear(&builder);
        return create_error_result(OPENPGP_ERROR_SERIALIZATION,
                                 "Failed to serialize SignDataBytesRequest");
    }
    
    /* Call bridge */
    BytesReturn* response = g_openpgp.bridge_call("signDataBytes", buffer, size);
...
openpgp_result_t openpgp_verify(const char *signed_message,
    ...
    *result = NULL;
    ...
    /* For signed messages, we should extract the original data */
    ...
    if (verify_result->is_valid) {
        ...
        // TODO: Parse the signed message to extract original content
        verify_result->original_data = duplicate_string("extracted content not yet implemented");
    }
...
openpgp_result_t openpgp_verify_data(const void *data,
    ...
    *result = NULL;
    ...
    /* For detached signatures, we don't extract original data since it's provided */
    if (verify_result->is_valid) {
        verify_result->signer_key_id = duplicate_string("unknown"); // TODO: Extract from signature
        verify_result->signer_fingerprint = duplicate_string("unknown"); // TODO: Extract from signature
        verify_result->original_data = NULL; // No original data for detached signatures
    }
...
openpgp_result_t openpgp_verify_file(const char *file_path,
    ...
    *result = NULL;
    ...
    /* Create string references */
    flatbuffers_string_ref_t signature_ref = flatbuffers_string_create_str(B, signature);
    flatbuffers_string_ref_t input_ref = flatbuffers_string_create_str(B, file_path);
    flatbuffers_string_ref_t public_key_ref = flatbuffers_string_create_str(B, public_key);
