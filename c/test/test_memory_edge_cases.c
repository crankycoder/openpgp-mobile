...
static int test_null_parameter_handling(void) {
    ...
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // NULL email may be allowed
    ...
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Empty message allowed
    
    result = openpgp_encrypt_symmetric("message", "", NULL, NULL);
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Empty password allowed
    
    result = openpgp_sign("", "key", NULL, NULL);
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Empty data allowed
    
    result = openpgp_sign("data", "", NULL, NULL);
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Empty key allowed
    
    result = openpgp_generate_key("", "email", "passphrase");
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Empty name allowed
    
    result = openpgp_generate_key("name", "", "passphrase");
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Empty email allowed
...
static int test_extreme_size_boundaries(void) {
    ...
    for (size_t i = 0; i < sizeof(special_messages) / sizeof(special_messages[0]); i++) {
        ...
        CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Should handle gracefully
    }
...
static int test_integer_overflow_scenarios(void) {
    ...
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Should be handled by bridge
    
    // Test with NULL passphrase
    result = openpgp_generate_key("Test User", "test@example.com", NULL);
    CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // NULL passphrase may be allowed
    ...
    if (test_msg) {
        ...
        CHECK_RESULT_AND_FREE(result, OPENPGP_ERROR_BRIDGE_CALL); // Should succeed allocation checks
