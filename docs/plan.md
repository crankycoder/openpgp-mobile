...
### Tasks:
1. **Create key generation integration test file**
   - File: `/c/test/test_keygen_integration.c`
   - Test framework for dogfooding generated keys
   - Helper functions for key validation

2. **Test RSA 2048 key generation and usage**
   - Generate RSA 2048-bit keypair without passphrase
...
3. **Test passphrase-protected key generation and usage**
   - Generate RSA 2048-bit keypair with passphrase
   - Test encryption with public key
   - Test decryption with private key + correct passphrase
   - Test decryption failure with wrong passphrase
   - Test decryption failure with no passphrase

4. **Test ECDSA P-256 key generation and usage**
   - Generate ECDSA P-256 keypair without passphrase
   - Test encryption/decryption round-trip
   - Generate ECDSA P-256 keypair with passphrase
   - Test passphrase-protected operations

5. **Test Ed25519 key generation and usage**
   - Generate Ed25519 keypair without passphrase
   - Test encryption/decryption round-trip
   - Generate Ed25519 keypair with passphrase
   - Test passphrase-protected operations

6. **Test multi-recipient scenarios with generated keys**
   - Generate multiple keypairs (mix of RSA, ECDSA, Ed25519)
   - Encrypt to multiple recipients
   - Verify each recipient can decrypt
   - Test mixed key types (RSA + ECDSA + Ed25519)

7. **Test key metadata extraction on generated keys**
   - Verify metadata extraction works on generated keys
   - Confirm key IDs, fingerprints are correct
   - Validate algorithm detection (RSA vs ECDSA vs Ed25519)
   - Check encryption/signing capabilities

8. **Add continuous integration validation**
...
- Test both armored and binary formats
- Validate against known good implementations
- Document any discovered limitations
- Ensure all three algorithms (RSA 2048, ECDSA P-256, Ed25519) are tested

### Verification:
- All generated keys must work for encryption/decryption
- Passphrase handling must be correct for all key types
...
## Phase 7: Verification Operations
